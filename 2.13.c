...
int number_of_optDFA;   /* number of new DFA states */
...
*/
int init_equiv_class(char StateName[][STATES+1], int N_DFA_states, char *DFA_finals)
{
    int i, j;
 
    if (strlen(DFA_finals) == N_DFA_states) {  /* all states are final states */
        strcpy(StateName[0], DFA_finals);
        return 1;
    }
 
    strcpy(StateName[1], DFA_finals);   /* final state group */
 
    for (i=j=0; i < N_DFA_states; i++) {
        if (i == *DFA_finals-'A') {
            DFA_finals++;
...
/*
    Get optimized DFA 'OptDFA' for equiv. class 'stnt'.
*/
// n2 = get_optimized_DFA(StateName, number_of_optDFA, DFAtab, N_symbols, OptDFA);
int get_optimized_DFA()
//char StateName[][STATES+1], int number_of_optDFA, int DFAtab[][SYMBOLS], int N_symbols, int newdfa[][SYMBOLS]
{
    int n2=number_of_optDFA;       /* 'n' + <num. of state-division info> */
    int i, j;
    char nextstate[STATES+1];
 
    for (i = 0; i < number_of_optDFA; i++) {    /* for each pseudo-DFA state */
        for (j = 0; j < N_symbols; j++) {    /* for each input symbol */
            get_next_state(nextstate, StateName[i], DFAtab, j);
            OptDFA[i][j] = state_index(nextstate, StateName, number_of_optDFA, &n2, i)+'A';
        }
    }
...
*/
//StateName, number_of_optDFA, OptDFA, N_symbols, N_DFA_states
int set_new_equiv_class()
{
    int state, symbol, k;
 
    for (state = 0; state < number_of_optDFA; state++) {
        for (symbol = 0; symbol < N_symbols; symbol++) {
            k = OptDFA[state][symbol]-'A';   /* index of equiv. vector */
            if (k >= number_of_optDFA)  /* equiv. class 'i' should be segmented */
                return split_equiv_class(StateName, state, k, number_of_optDFA, N_DFA_states);
        }
    }
 
    return number_of_optDFA;
}
 
void print_equiv_classes(char StateName[][STATES+1], int number_of_optDFA)
{
    int i;
 
    printf("\nEQUIV. CLASS CANDIDATE ==>");
    for (i = 0; i < number_of_optDFA; i++)
...
*/
int optimize_DFA()  /* reduced DFA table */
{//N_DFA_states, N_symbols, DFA_finals, StateName, OptDFA
    char nextstate[STATES+1];
    int n2; /* 'number_of_optDFA' + <num. of state-dividing info> */
 
    number_of_optDFA = init_equiv_class(StateName, N_DFA_states, DFA_finals);
 
    while (1) {
        print_equiv_classes(StateName, number_of_optDFA);
        n2 = get_optimized_DFA();
        if (number_of_optDFA != n2)
            number_of_optDFA = set_new_equiv_class();
        else break; /* equiv. class segmentation ended!!! */
    }
 
    return number_of_optDFA;   /* number of DFA states */
}
